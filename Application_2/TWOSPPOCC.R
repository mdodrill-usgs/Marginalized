...
#Get data and format for models listed  below:
...
######################################################################
## JAGS-Discrete
### For each species, two states: 0 - absent; 1 - present and two sets of process variables - colonization and extinction
# requires matrix of visited per site/year/8 visits with 0 when not visited and 1 when visited and y
...
model {
	...
	psi0_BO~dunif(0,1)
	psi0_NSO~dunif(0,1)
	for (t in 1:21){gamNSO_int[t]~dunif(-5,5)} # time varying colonization
	...
	for (t in 1:21){
		st_psiBO[t]<-(sum(BO[,t])/158-.5) #approximately centered
		for (k in 1:158){
			...
			BO[k,(t+1)]~dbern(gamBO[k,t]*(1-BO[k,t])+(1-epsBO[k,t])*BO[k,t]) #probability BO is probability of BO occupancy 
...
owls_JD.data<-list (Y_BO=array(Y_BO,dim=c(dim(Y_BO)[1],dim(Y_BO)[2])),Y_NSO=array(Y_NSO,dim=c(dim(Y_NSO)[1],dim(Y_NSO)[2])),
			  BO=array(BO,dim=c(dim(BO)[1],dim(BO)[2])),NSO=array(NSO,dim=c(dim(NSO)[1],dim(NSO)[2])),
...
owls_JD.par<-c('pBO_b','pNSO_b','psi0_BO','psi0_NSO','gamNSO_b','epsNSO_b','epsNSO_int','gamBO_int','epsBO_int','gamBO_b','epsBO_b', 'gamNSO_int')
owls_JD.out<- jags.parallel(owls_JD.data,inits=NULL,owls_JD.par,"owls_JD.txt",n.chains = 3,n.iter=2000, jags.seed=1)


######################################################################
# BUGS-Marginalized - 4 states: 1 - Neither present, 2 - NSO present, 3 - BO present, 4 - both species present
...
model {
	...
	for (k in 1:158){
		for (j in 1:176){
			...
			p[k,j,1,3]<-0
			p[k,j,1,4]<-0
			p[k,j,2,1]<-1-pNSO_bo[k,j]
			p[k,j,2,2]<-pNSO_bo[k,j]
			p[k,j,2,3]<-0
			p[k,j,2,4]<-0
			p[k,j,3,1]<-1-pBO[k,j]
			p[k,j,3,2]<-0
			p[k,j,3,3]<-pBO[k,j]
			p[k,j,3,4]<-0
			p[k,j,4,1]<-1-(pBO[k,j]+pNSO_BO[k,j]-pBO[k,j]*pNSO_BO[k,j])
			p[k,j,4,2]<-pNSO_BO[k,j]*(1-pBO[k,j])
			p[k,j,4,3]<-(1-pNSO_BO[k,j])*pBO[k,j]
			p[k,j,4,4]<-pNSO_BO[k,j]*pBO[k,j]
			}
		for (t in 1:22){
	...
	for (t in 1:21){
		st_psiBO[t]<-(sum(BO[1:158,t])/158-.5)
		for (k in 1:158){
			logit(gamNSO[k,t])<-gamNSO_int[t]+gamNSO_b*FOR[k,t]
...
owls_JM.out<- jags.parallel(owls_JM.data,inits=NULL,owls_JM.par,".\\owls_JM.txt",n.chains = 3,n.iter=1000)
...
transformed parameters {
	...
	for (t in 1:21){
		...
		for (k in 1:158){
			...
			pz[k,(t+1),1]=dot_product(pz[k,t,],tr[,1])*cp[k,(t+1),1];
			pz[k,(t+1),2]=dot_product(pz[k,t,],tr[,2])*cp[k,(t+1),2];
			pz[k,(t+1),3]=dot_product(pz[k,t,],tr[,3])*cp[k,(t+1),3];
			pz[k,(t+1),4]=dot_product(pz[k,t,],tr[,4])*cp[k,(t+1),4];
...
owls_SM.out<- stan(".\\owls_SMRE_v2.stan",data=owls_SM.data,pars=owls_SM.par,chains = 3,iter=1000,seed=1)


###############################################################
#Stan Marginalized model with random effects - same parameterization as Stan Marginalized model
#Also, includes additive spatial and temporal random effects on extinction and colonization probabilities
#Code calculates multi-level R^2 values

sink("owl_SMRE_additive.stan")
cat("
data {
	int<lower=1,upper=4> Y[158,176];
	real RF [158,21];
...
owls_SMRE.par<-c('pBO_b','pNSO_b','psi0_BO','psi0_NSO','gamNSO_b','epsNSO_b','epsNSO_int','gamBO_int','epsBO_int','gamBO_RF','gamBO_autolog', 
		   'gamBO_xNSO', 'epsBO_RF', 'epsBO_autolog','epsBO_xNSO', 'gamNSO_int', 'R2_gamBO_site', 'R2_gamBO_time', 'R2_gamNSO_site','R2_gamNSO_time','R2_epsBO_site', R2_epsBO_time', 'R2_epsNSO_site' , 'R2_epsNSO_time')
		 
owls_SMRE.out<- stan(".\\owls_SMRE_additive.stan",data=owls_SMRE.data,pars=owls_SMRE.par,chains = 3,iter=1000,seed=i)

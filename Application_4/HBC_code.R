...
for (i in 1:3){
	...
	# tr is an array representing markov transitions for three intervals. some transitions are assumed to be zero (e.g., transitioning from a large to a small adult)
...
model {
   ...
   vector[7] pz[27]; 
	for (k in 1:NsumCH) { 
       		for (j in 1:6) {pz[sumf[k], j] = (1-0.03*newtag[k])*(j == sumCH[k, sumf[k]]); } 
...
transformed parameters {
	...
	for (j in 1:4){for(i in 1:26){ls[i,j]=mu_ls[j]+z_ls[i,j]*sd_ls;}}
	for (j in 1:4){for(i in 1:26){lm[i,j]=mu_lm[season[i],j]+z_lm[i,j]*sd_lm;}}
	for (j in 1:2){for(i in 1:26){lg[i,j]=mu_lg[season[i],j]+z_lg[i,j]*sd_lg;}}   
...
model {
	...
	for(i in 1:26){z_ls[i]~normal(0,1);
		       z_lg[i]~normal(0,1);
		       z_lm[i]~normal(0,1);}

	mu_ls~normal(0,2);

	for(i in 1:3){mu_lm[i]~normal(0,2);
		      mu_lg[i]~normal(0,2);}

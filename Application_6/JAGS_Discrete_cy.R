...
#
###############################################################################
setwd("U:\\Desktop\\Fish_Git\\Marginalized\\Application_6")
...
model{
  ...
  for(k in 1:NindCH){
    pz[k,indf[k]] <- indCH[k,indf[k]]    
    for(j in indf[k]:(indlast[k] - 1)){
      pz[k,(j + 1)] ~ dcat(btrans[pz[k,j], ,seasNO[(j + 1)]])
      indCH[k,(j + 1)] ~ dcat(bp[j,pz[k,(j + 1)],])
    }
  }
...
initator <- function(ch){
  ...
  for(i in 1:nrow(ch)){
    if(ch[i,f[i]] == ch[i,l[i]]){
      ch[i,f[i]:l[i]] = ch[i,f[i]]
    } else {
      st = ch[i,f[i]]
      end = ch[i,l[i]]
      n.occ = length(f[i]:l[i])
      
      ch[i,f[i]:l[i]] = as.integer(seq(st, end, length.out = n.occ))
      # ch[i,(l[i]-1)] = ch[i,l[i]]
    }
  }
...
init <- function() list(pz = initator(indCH))
...
# t1 <- proc.time()
# jags.fit <- jags.parallel(BM_JM.data, inits = NULL, BM_JM.par, "JAGS_Discrete.jags",
jags.fit <- jags.parallel(BM_JM.data, inits = init, BM_JM.par, "JAGS_Discrete.jags",
                          n.chains = 1, n.iter = ni, export_obj_names = c("ni", "initator"))
# t2 <- proc.time()
#-----------------------------------------------------------------------------#

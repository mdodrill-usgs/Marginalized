...
sm.data <- list(NAZsamps  =  NAZsamps, ts = ts, AZeff = AZeff,bAZ = bAZ,seasNO = seasNO,bNOc = bNOc,
              NOpasses = NOpasses,NCH = NCH,FR = FR,last = last,bCH = bCH,sumf = sumf,spawn = spawn)
...
# Call Stan from R 
SM.c <- stan("nop_nore.stan",
             data = sm.data,
             pars = sm.params,
             control = list(max_treedepth = 14, adapt_delta = .85),
             chains = nc, iter = ni, thin = nt, seed = 1) 

# SM_nopnore<-stan("nop_nore.stan",data=SM_data,pars=params,chains=3,iter=1000) 
...
my.n.iter = c(1000, 2500, 5000, 7500, 10000)
...
out <- foreach(j = seeds) %:%
  ...
  foreach(i = my.n.iter) %dopar% {
    ...
    SM.c <- stan("Stan_Marginalized_no_prior_no_re.stan",
                 data = sm.data,
                 pars = sm.params,
                 # control = list(max_treedepth = 14, adapt_delta = .85),
...
all.stan.m.no.prior.no.re = all.out
rm(list=setdiff(ls(), c("all.stan.m.no.prior.no.re")))

save.image("U:/Desktop/Fish_Git/Marginalized/Application_6/working_runs/Stan_run_no_prior_no_re.RData")
#-----------------------------------------------------------------------------#
# end
